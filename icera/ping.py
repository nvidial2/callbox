...
class Pinger:
	...
	self.pid = os.getpid()
	self.addr = addr
	name, aliases, ipaddr = socket.gethostbyname_ex(addr)
	if aliases:
	...
	return {'dmin':dmin, 'davg':davg, 'dmax':dmax, 'sent':sent, 'recv':recv, 'loss':loss}
...
def ping(who, num):
    ...
    print "%d packets transmitted, %d packets received, %d%% packet loss" % \
	  (summary['sent'], summary['recv'], int(summary['loss'] * 100.))
    print "round-trip (ms)   min/avg/max = %d/%d/%d" % \
	  (summary['dmin'], summary['davg'], summary['dmax'])
    return summary
