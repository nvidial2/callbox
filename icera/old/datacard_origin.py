...
class SerialPort:
    ...
    def wait_csattach_csdetach(self,type=1,timeout=60):
        #type == 1 to wait for CS attachment
        #        0 to wait for CS detachement
        comment="Failed"
        index=0
        a=time.time()
        b=time.time()
        while (b-a)<=timeout:
            index+=1
            if type==1 and self.is_csattach():
                b=time.time()
                comment="Attach (%d sec)" % (b-a,)
                return True
                break
            elif type==0 and not self.is_csattach():
                b=time.time()
    ...
    def is_csattach(self):
        reponse=self.send('at+creg?')
        
        if re.search('CREG: 0,1',reponse):
            return True
        else:
            return False
    ...
    def wait_camping_on_2G_cell(self,type=1,timeout=120):
        #type == 1 to wait for 2G camping
        #        0 to wait for 2G camping
        comment="Failed"
        index=0
        a=time.time()
        b=time.time()
        while (b-a)<=timeout:
            index+=1
            if type==1 and self.is_campedon2Gcell():
                b=time.time()
                comment="Camped on 2G cell (%d sec)" % (b-a,)
                return True
                break
            elif type==0 and not self.is_campedon2Gcell():
                b=time.time()
                comment="Not Camped on 2G cell(%d sec)" % (b-a,)
                return True
                break
            else:
                b=time.time()
    ...
    def wait_camping_on_3G_cell(self,type=1,timeout=60):
        #type == 1 to wait for 3G camping
        #        0 to wait for 3G camping
        comment="Failed"
        index=0
        a=time.time()
        b=time.time()
        while (b-a)<=timeout:
            index+=1
            if type==1 and self.is_campedon3Gcell():
                b=time.time()
                comment="Camped on 3G cell (%d sec)" % (b-a,)
                return True
                break
            elif type==0 and not self.is_campedon3Gcell():
                b=time.time()
                comment="Not Camped on 3G cell(%d sec)" % (b-a,)
                return True
                break
            else:
                b=time.time()
    ...
    def is_campedon2Gcell(self):
        reponse=self.send('at%nwstate=1')
        
        if re.search('GSM',reponse):
            return True
        else:
            return False
        
    def is_campedon3Gcell(self):
        reponse=self.send('at%nwstate=1')
        
        if re.search('3G',reponse):
            return True
        else:
            return False        
